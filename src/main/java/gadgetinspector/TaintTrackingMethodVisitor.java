...
//                break;
                pop();              // 弹出 index
                saved0 = pop();     // 弹出 array reference, 并保存它的污点 (saved0)
                push(saved0);       // 将数组的污点作为元素的污点压入栈中
                break;
            case Opcodes.LALOAD:
            case Opcodes.DALOAD:
//                pop();
...
//                break;
                pop();              // 弹出 index
                saved0 = pop();     // 弹出 array reference, 并保存它的污点 (saved0)
                push(saved0);       // 将污点压入栈中 (占一个槽位)
                push(saved0);       // 再次压入，因为 long/double 占两个槽位
                break;
    ...
    public void visitMethodInsn(int opcode, String owner, String name, String desc, boolean itf) {
        ...
        switch (opcode) {
            ...
            case Opcodes.INVOKEINTERFACE:
                ...
                // ==================== 新增优化代码 START ====================
                // 优化：特殊处理 InvocationHandler.invoke 方法的污点传递
                // 如果一个类实现了 InvocationHandler 接口，并且正在调用 invoke 方法，
                // 那么其返回值（Object）的污点应该来自于第三个参数（Object[] args）。
                if (name.equals("invoke") && desc.equals("(Ljava/lang/Object;Ljava/lang/reflect/Method;[Ljava/lang/Object;)Ljava/lang/Object;")) {
                    Set<ClassReference.Handle> parents = inheritanceMap.getSuperClasses(new ClassReference.Handle(owner));
                    if (parents != null && parents.contains(new ClassReference.Handle("java/lang/reflect/InvocationHandler"))) {
                        // 第三个参数的索引是 3 (0: this, 1: proxy, 2: method, 3: args)
                        if (argTaint.size() > 3) {
                            resultTaint.addAll(argTaint.get(3));
                        }
                    }
                }
                // ==================== 新增优化代码 END ======================
